...
odd_h = c1.number_input("Odd Casa",   min_value=1.01, value=2.00, step=0.01)
odd_d = c2.number_input("Odd Empate", min_value=1.01, value=3.50, step=0.01)
odd_a = c3.number_input("Odd Fora",   min_value=1.01, value=4.00, step=0.01)
...
if st.button("Verificar", use_container_width=True):
    ...
    if lay_v and lay_0x1:
        ...
        st.success("✅ Lay 0x1")
        st.info("📌 **Lay 0x1 — filtro pré-jogo.** Confirme campo: chutes ou escanteios a favor da casa. Odd de entrada **≤ 25**.")

    elif lay_v:
        st.success("✅ Lay Visitante")
        if 3.30 <= odd_a <= 5.00:
            st.info("📌 **Punter:** odd de entrada dentro da faixa ideal — 3.30 a 5.00.")
        else:
            st.warning(f"⚠️ Padrão batido, mas odd de entrada ({odd_a:.2f}) fora da faixa ideal (3.30 – 5.00).")
